...
try:
    ...
    found = []
    for e in entities:
        name = (e.get("LogicalName") or "").lower()
        coll = (e.get("LogicalCollectionName") or "").lower()
        label = (((e.get("DisplayName") or {}).get("UserLocalizedLabel") or {}).get("Label") or "").lower()
    ...
    if not found:
        ...
        for e in entities:
            if (e.get("LogicalName") or "").startswith("tyr_"):
